...
//					throw new SQLNonTransientException(msg);
					...
					// sxl add distTable
					if(tc.isDistTable()) {
						parserBatchInsertDistTable(schema, rrs, partitionColumn, tableName, insert);
					} else {
	 ...
	 * @throws SQLNonTransientException
	 */
	private void parserBatchInsertDistTable(SchemaConfig schema, RouteResultset rrs, String partitionColumn,
											String tableName, MySqlInsertStatement insertStmt) throws SQLNonTransientException {
		...
		if(insertStmt.getValuesList().size() > 1) {
			...
			if(shardingColIndex == -1) {
				...
				for(ValuesClause valueClause : valueClauseList) {
					...
					Integer tableIndex = algorithm.calculate(shardingValue);
					//没找到插入的分片
					if(tableIndex == null) {
						String msg = "can't find any valid tableIndex :" + tableName
								+ " -> " + partitionColumn + " -> " + shardingValue;
						LOGGER.warn(msg);
						throw new SQLNonTransientException(msg);
					}
					...
					if(subTable != null) {
						...
						String msg = "can't find subTable:" + tableName
								+ " -> " + partitionColumn + " -> " + shardingValue + " -> " + tableIndex;
						LOGGER.warn(msg);
						throw new SQLNonTransientException(msg);
					}
				}
		...
		if(tablesRouteSet.size() > 1) {
			String msg = "TODO:insert into .... values .... find muti tables, not supported!";
			LOGGER.warn(msg);
			throw new SQLNonTransientException(msg);
		}
		...
		String table = String.valueOf(subTables[0]);
		String changeSql = insertStmt.toString();
		nodes[0] = new RouteResultsetNode(tableConfig.getTableDataNodeMap().get(table), rrs.getSqlType(), changeSql);//rrs.getStatement()
		nodes[0].setSubTableName(table);

		MySqlInsertStatement stat = insertStmt;
		SQLTableSource tableSource  = stat.getTableSource();
		SQLExprTableSource from2 = getDisTable(tableSource, nodes[0]);
		stat.setTableSource(from2);
		nodes[0].setStatement(stat.toString());
