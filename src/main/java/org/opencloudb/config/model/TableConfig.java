...
import org.opencloudb.route.function.dbtable.PartitionByDbAndTable;
...
public class TableConfig {
	...
	private final Map<String, String> tableDataNodeMap = Maps.newHashMap();
	...
	public TableConfig(String name, String primaryKey, boolean autoIncrement,boolean needAddLimit, int tableType,
		...
		if(subTables!=null && !subTables.equals("")){
			...
			// sxl 增加多库分表支持
			int perDb = 0;
			if (rule.getRuleAlgorithm() instanceof PartitionByDbAndTable) {
				PartitionByDbAndTable dtRule = (PartitionByDbAndTable) rule.getRuleAlgorithm();
				perDb = dtRule.getTablesByPerDataBase();
				if(sTables.length != dtRule.getTotalTables()) {
					throw new IllegalArgumentException("tables not match! subTables -> " + sTables.length + ", rule tables -> " + dtRule.getTotalTables());
				}
				if(dataNodes.size() != dtRule.getTotalDataBases()) {
					throw new IllegalArgumentException("dataNodes not match! dataNodes -> " + dataNodes.size() + ", rule dbs -> " + dtRule.getTotalDataBases());
				}
			} else {
				throw new IllegalArgumentException("only PartitionByDbAndTable Algorithm support!");
			}

			for (int i=0; i<sTables.length; i++) {
				if(dataNodes.size() == 1) {
					tableDataNodeMap.put(sTables[i], dataNodes.get(0));
				}else {
					int dataNodeIdx = i / perDb;
					tableDataNodeMap.put(sTables[i], dataNodes.get(dataNodeIdx));
				}
			}
		}else{
	...
	public Map<String, String> getTableDataNodeMap() {
		return tableDataNodeMap;
	}
}